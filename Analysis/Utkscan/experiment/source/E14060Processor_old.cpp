...
//!< and I2S
        ...
        const int DD_CORR_DECAY_PSPMT = 30;
...
void E14060Processor::DeclarePlots(void) {
    ...
    DeclareHistogram2D(DD_CORR_DECAY_PSPMT, S5, S5, "Decay pixels correlated with Implants");
...
bool E14060Processor::Process(RawEvent &event) {
   ...
   if (pin1 != 0 && pin2 != 0 && delta < 40 && delta > -40)
    ...
    int decay_window = 3 * 59e-3 / Globals::get()->GetEventLengthInSeconds();

    int pixel_time [Px][Py] ={};
    ...
    plot(DD_CORR_DECAY_PSPMT, pixel.first + 1, pixel.second + 1);
